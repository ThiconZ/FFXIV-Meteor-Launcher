// C# reimplementation and fixes of the SeventhUmbral BLOWFISH library
// https://github.com/jpd002/SeventhUmbral/blob/master/common/BLOWFISH.C
    ...
    public class Blowfish
    {
        ...
        public Blowfish(string key)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                int x = i * 1024;
                S[i] = new uint[256];
                Buffer.BlockCopy(S_values, i * 1024, S[i], 0, 1024);
            }
        ...
        {
            uint y;
            UintByteBreaker bytes = new UintByteBreaker();
            bytes.UInt = x;

            y = S[0][bytes.Byte4] + S[1][bytes.Byte3];
            y = y ^ S[2][bytes.Byte2];
            y = y + S[3][bytes.Byte1];
        ...
        public (uint l, uint r) Encrypt(uint xl, uint xr)
        {
            uint temp;
            short i;
